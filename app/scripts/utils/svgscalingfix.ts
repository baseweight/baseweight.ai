// Utility to calculate minimum window substring
export function minimumWindowSubstring(str: string, subStr: string): string {
  ...
  for (; fastPointer < str.length; fastPointer++) {
    ...
    while (missingCharsCounter === 0) {
      ...
      if (
        ...
        shortestWindow[1] - shortestWindow[0]
      ) {
...
// Function to generate a scale transform string
export function svgScale(scaleFactor: number): string {
